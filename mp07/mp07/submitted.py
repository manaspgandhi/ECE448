...
def minimax(board, side, flags, depth):
    ...
    def exploreMovesMM(board, side, flags, depth, moveTree):
        possibleMoves = generateMoves(board, side, flags)
        if depth == 0 or not possibleMoves:
            return evaluate(board), []
        
        addedMoves = []
        
        #maximize, means player1 is currently playing
        if side == False:
            #negative infinity cuz maximizing
            moveScore = float('-inf')
            
            for currMove in possibleMoves:
                #update move tree
                moveTree[encode(*currMove)] = {}
                
                # make the current move and get the board status
                nextSide, nextBoard, nextFlags = makeMove(side, board, currMove[0], currMove[1], flags, currMove[2])
                nextDepth = depth - 1
                
                #update moveTree and run the next explore moves
                
                newMoveScore, moveList = exploreMovesMM(nextBoard, nextSide, nextFlags, nextDepth, moveTree[encode(*currMove)])
                
                # add addedMoves if it works and update score
                if moveScore < newMoveScore:
                    moveScore = newMoveScore
                    addedMoves = [currMove] + moveList
                    
            return moveScore, addedMoves
        
        #minimize, means player1 is currently playing
        else:
            #infinity cuz minimizing
            moveScore = float('inf')
    ...
    moveScore, moveList = exploreMovesMM(board, side, flags, depth, moveTree)
    return (moveList, moveTree, moveScore)
...
def alphabeta(board, side, flags, depth, alpha=-math.inf, beta=math.inf):
    ...
    def exploreMovesAB(board, side, flags, depth, a, b, moveTree):
        possibleMoves = generateMoves(board, side, flags)
        ...
        if side == False:
            ...
            for currMove in possibleMoves:
                ...
                nextMoveDepth = depth - 1
                newMoveScore, moveList = exploreMovesAB(nextBoard, nextSide, nextFlags, nextMoveDepth, a, b, moveTree[encode(*currMove)])
    ...
    moveScore, moveList = exploreMovesAB(board, side, flags, depth, alpha, beta, moveTree)
    return (moveList, moveTree, moveScore)
