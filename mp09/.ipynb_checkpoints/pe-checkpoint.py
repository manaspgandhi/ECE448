    ...
    def __init__(self, d_model, max_seq_length):
        ...
        for position in range(max_seq_length):
            ...
            for dim in range(d_model):
                ...
                if dim % 2 == 0:
                    position_coefficient = math.exp((dim * -math.log(10000))/d_model)
                    pe_val = math.sin(position * position_coefficient)
                else:
                    position_coefficient = math.exp(((dim-1) * -math.log(10000))/d_model)
                    pe_val = math.cos(position * position_coefficient)
                pe_values.append(pe_val)
                
            pe[position] = torch.Tensor(pe_values)
