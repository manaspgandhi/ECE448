    ...
    def test_decoder_inference_cache_extra_credit(self):
        ...
        for item_idx in range(len(self.decoder_inference_test_data["inference_decoder_cache"])):
            ...
            for l_idx in range(len(ref_decoder_cache_item)):
                self.assertAlmostEqual(torch.max(torch.abs(decoder_cache_item[l_idx] - ref_decoder_cache_item[l_idx])).item(), 0, places = 2, msg=f'Decoder cache during inference differs at layer #{l_idx}, for sample #{item_idx}')