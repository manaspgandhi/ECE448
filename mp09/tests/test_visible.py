        ...
        with torch.no_grad():
            ...
            for it_idx, (src, trg, src_lengths, trg_lengths) in enumerate(data_iter):
                ...
                for item_idx in range(cur_ref_batch_output_encoder.size(0)):
                    ...
                    self.assertAlmostEqual(torch.max(torch.abs(ref_enc_item - enc_item)).item(), 0, places = 2, msg=f'The encoder output for the sample #{item_idx} in the batch #{it_idx} is not correct')
        ...
        with torch.no_grad():
            ...
            for it_idx, (src, trg, src_lengths, trg_lengths) in enumerate(data_iter):
                ...
                for item_idx in range(ref_output.size(0)):
                    ...
                    self.assertAlmostEqual(torch.max(torch.abs(ref_dec_item - dec_item)).item(), 0, places=2, msg=f"The decoder output for the sample #{item_idx} in the batch #{it_idx} is not correct")
