...
#         future_return = rollout_buffer.rewards[i] + (discount_factor * future_return) 
#         returns.insert(0, future_return)  # insert at the beginning cuz order
 
#     torch_tensor = torch.tensor(returns, dtype=torch.float32)
    
    returns = torch.zeros(len(rollout_buffer.actions))
    future_return = 0.0
    T = len(rollout_buffer.actions)
    
    for t in range(len(rollout_buffer.rewards) - 1, -1, -1):
        if rollout_buffer.terminateds[t]:
            T = t + 1
            future_return = rollout_buffer.rewards[t]
            returns[t] = future_return
        else:
            for i in range(t, T):
                future_return = rollout_buffer.rewards[i] * np.power(discount_factor, (i - t)) #r_i + (gamma * next_return)
                returns[t] += future_return
                
    tensor = torch.Tensor(returns).unsqueeze(1)
...
def get_advantages(value_net: nn.Module,
    ...
    with torch.no_grad():
        value_predictions = value_net(observations)
    
    advantage = returns - value_predictions
    mean_advantages = torch.mean(advantage)
    std_advantages = torch.std(advantage) + EPSILON
    standardized_advantages = (advantage - mean_advantages) / std_advantages
...
def get_value_net_loss(value_net: nn.Module,
    ...
    """
    # YOUR CODE HERE
    predicted_vals = value_net(observation)
    loss = torch.nn.functional.mse_loss(predicted_vals, returns) 
    return loss
...
def get_vanilla_policy_gradient_loss(policy: nn.Module,
    ...
    """
    # YOUR CODE HERE
    action_log_probs = policy(observation)
    selected_actions_log_probs = action_log_probs.gather(1, action.long())
    loss = -torch.mean(selected_actions_log_probs * return_or_advantage)
    ...
    while r < rollouts:
        ...
        for _ in range(training_epochs_per_rollout):
            ...
            for batch_start in range(0, rollout_buffer.final_size, minibatch_size):
                ...
                # use idxr_base[batch_start: batch_stop] to get the batch
                curr_batch = idxr_base[batch_start: batch_stop]
                
                observation = rollout_buffer.observations[curr_batch]
                old_logits = rollout_buffer.old_logits[curr_batch]
                action = rollout_buffer.actions[curr_batch]
                
                if get_advantages != None: # if get advantages is provided
                    return_or_advantage = advantages[curr_batch]
                else: # if not provided
                    return_or_advantage = returns[curr_batch]
                
                returns_kwargs = returns[curr_batch]
                ...
                policy_gradient_kwargs = dict(
                    ...
                    critic=                 None, # Fill in
                    observation=            observation, # Fill in
                    old_logits=             old_logits, # Fill in
                    action=                 action, # Fill in
                    return_or_advantage=    return_or_advantage, # Fill in
                    returns=                returns_kwargs, # Fill in 
                    ppo_clip=               ppo_clip
                )
